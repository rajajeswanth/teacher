...
resource "aws_ecs_service" "main1" {
  ...
  depends_on = [aws_alb_listener.front_end1, aws_iam_role_policy_attachment.ecs_task_execution_role]
}
...
resource "aws_ecs_service" "main2" {
  ...
  depends_on = [aws_alb_listener.front_end2, aws_iam_role_policy_attachment.ecs_task_execution_role]
}
